// 83C661X2 processor definition file
// ==================================
// Philips P83C661X2 and P87C661X2
...
var
  ...
  S2STA     : byte absolute $F1;
  S2DAT     : byte absolute $F2;
  S2ADR     : byte absolute $F3;
  S2IST     : byte absolute $F4;  // probably doesn't exist
  ...
  CR0_2     : boolean absolute $F8;  // S2CON
  CR1_2     : boolean absolute $F9;
  AA_2      : boolean absolute $FA;  // Philips uses the same names for S1CON and S2CON
  SI_2      : boolean absolute $FB;  // register bits. So I added the suffix _2 to the
  STO_2     : boolean absolute $FC;  // S2CON bit names, just to make things unique.
  STA_2     : boolean absolute $FD;
  ENS1_2    : boolean absolute $FE;  // sometimes also named ENA1 in the Philips data sheet
  CR2_2     : boolean absolute $FF;
