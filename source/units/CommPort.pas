...
{------------------------------------------------------------------- 
  Enhancements:
  New Methode:   ToggleTXD  (using break function)
    J. Rathlev, Aug. 2004   (kontakt(a)rathlev-home.de)
  Changes for Delphi 2009
    J. Rathlev, May  2009   (kontakt(a)rathlev-home.de)
                Jan. 2016 : removed from components, use as unit
                            renamed to ComPort.pas
  ...
  -------
    ...
    begin
      ...
    // Create TCommPortDriver object
      ComPort:=TCommPortDriver.Create(self);
      with ComPort do begin
        Port:=pnCustom;
        PortName:='\\.\COM2';
        OnReceiveData:=ComPortReceiveData; // user defined function
        end;      
      ...     
      end;
    ...
    begin
      ...
    // Release TCommPortDriver object
      ComPort.Free;
      end;
...
type
  ...
  protected
    ...
    // errror status on connection
    FErrorStatus               : TCommError;
  ...
  public
    ...
    // Set TXD line high (onOff=TRUE) or low (onOff=FALSE).
    // You must not use HW handshaking.
    // *** added: J. Rathlev, Aug. 2004
    ...
    // errror status on connection
    property ErrorStatus : TCommError read FErrorStatus;
...
begin
  ...
  with FErrorStatus do begin
    ErrorCode:=NoError;
    ErrorMsg:=rsComNoError;
    end;
...
begin
  ...
  if not Result then begin
    with FErrorStatus do begin
      ErrorCode:=GetLastError;
      ErrorMsg:=rsComFlushError;
      end;
    exit;
    end;
  // Setup buffers size
  Result := SetupComm( FHandle, FInBufSize, FOutBufSize );
  if not Result then with FErrorStatus do begin
    ErrorCode:=GetLastError;
    ErrorMsg:=rsComSetupError;
    end;
end;
...
const funcs: array[boolean] of integer = (CLRBREAK,SETBREAK);
begin
  if Connected then
    EscapeCommFunction( FHandle, funcs[onOff] );
end;
