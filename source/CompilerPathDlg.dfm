object CompilerPathDialog: TCompilerPathDialog
  Left = 195
  Top = 108
  BorderStyle = bsDialog
  Caption = 'Path settings'
  ClientHeight = 290
  ClientWidth = 411
  Color = clBtnFace
  ParentFont = True
  OldCreateOrder = True
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 411
    Height = 249
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 5
    ParentColor = True
    TabOrder = 0
    object pcCompiler: TPageControl
      Left = 5
      Top = 5
      Width = 401
      Height = 239
      ActivePage = tsPascal
      Align = alClient
      TabOrder = 0
      object tsAssembler: TTabSheet
        Caption = 'Assembler'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        DesignSize = (
          393
          211)
        object btnOutPath: TSpeedButton
          Left = 359
          Top = 180
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0}
          NumGlyphs = 2
          OnClick = btnOutPathClick
          ExplicitLeft = 360
        end
        object btnIncludePath: TSpeedButton
          Left = 359
          Top = 80
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0}
          NumGlyphs = 2
          OnClick = btnIncludePathClick
          ExplicitLeft = 360
        end
        object btnListPath: TSpeedButton
          Left = 359
          Top = 130
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0}
          NumGlyphs = 2
          OnClick = btnListPathClick
          ExplicitLeft = 360
        end
        object btnAsmModPath: TSpeedButton
          Left = 359
          Top = 30
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0}
          NumGlyphs = 2
          OnClick = btnAsmModPathClick
          ExplicitLeft = 360
        end
        object leOutPath: TLabeledEdit
          Left = 5
          Top = 180
          Width = 346
          Height = 21
          EditLabel.Width = 144
          EditLabel.Height = 13
          EditLabel.Caption = 'Output path for HEX and OMF'
          TabOrder = 0
        end
        object leIncludePath: TLabeledEdit
          Left = 5
          Top = 80
          Width = 346
          Height = 21
          EditLabel.Width = 65
          EditLabel.Height = 13
          EditLabel.Caption = 'Include paths'
          TabOrder = 1
        end
        object leListPath: TLabeledEdit
          Left = 5
          Top = 130
          Width = 346
          Height = 21
          EditLabel.Width = 74
          EditLabel.Height = 13
          EditLabel.Caption = 'Path for listings'
          TabOrder = 2
        end
        object leAsmModPath: TLabeledEdit
          Left = 5
          Top = 30
          Width = 346
          Height = 21
          EditLabel.Width = 64
          EditLabel.Height = 13
          EditLabel.Caption = 'Modules path'
          TabOrder = 3
        end
      end
      object tsPascal: TTabSheet
        Caption = 'Pascal'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        DesignSize = (
          393
          211)
        object btnPasUnitPath: TSpeedButton
          Left = 360
          Top = 130
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnPasUnitPathClick
        end
        object btnPasIncludePath: TSpeedButton
          Left = 360
          Top = 80
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnPasIncludePathClick
        end
        object btnPasOutPath: TSpeedButton
          Left = 360
          Top = 180
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnPasOutPathClick
        end
        object btnPasModPath: TSpeedButton
          Left = 360
          Top = 30
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnOutPathClick
        end
        object lePasUnitPath: TLabeledEdit
          Left = 5
          Top = 130
          Width = 346
          Height = 21
          EditLabel.Width = 49
          EditLabel.Height = 13
          EditLabel.Caption = 'Unit paths'
          TabOrder = 0
        end
        object lePasIncludePath: TLabeledEdit
          Left = 5
          Top = 80
          Width = 346
          Height = 21
          EditLabel.Width = 65
          EditLabel.Height = 13
          EditLabel.Caption = 'Include paths'
          TabOrder = 1
        end
        object lePasOutPath: TLabeledEdit
          Left = 5
          Top = 180
          Width = 346
          Height = 21
          EditLabel.Width = 198
          EditLabel.Height = 13
          EditLabel.Caption = 'Output path for HEX, OMF, ASM and U51'
          TabOrder = 2
        end
        object lePasModPath: TLabeledEdit
          Left = 5
          Top = 30
          Width = 346
          Height = 21
          EditLabel.Width = 64
          EditLabel.Height = 13
          EditLabel.Caption = 'Modules path'
          TabOrder = 3
        end
      end
      object tsCpp: TTabSheet
        Caption = 'C'
        ImageIndex = 2
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        DesignSize = (
          393
          211)
        object btnCppIncludePath: TSpeedButton
          Left = 359
          Top = 80
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnCppIncludePathClick
          ExplicitLeft = 360
        end
        object btnCppLibPath: TSpeedButton
          Left = 359
          Top = 130
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnCppLibPathClick
          ExplicitLeft = 360
        end
        object btnCppOutPath: TSpeedButton
          Left = 359
          Top = 180
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnCppOutPathClick
          ExplicitLeft = 360
        end
        object btnCppModPath: TSpeedButton
          Left = 359
          Top = 30
          Width = 21
          Height = 21
          Anchors = [akTop, akRight]
          Glyph.Data = {
            
            333373F773333333333330033333333333333773333333333333}
          NumGlyphs = 2
          OnClick = btnOutPathClick
          ExplicitLeft = 360
        end
        object leCppIncludePath: TLabeledEdit
          Left = 5
          Top = 80
          Width = 346
          Height = 21
          EditLabel.Width = 113
          EditLabel.Height = 13
          EditLabel.Caption = 'Additional include paths'
          TabOrder = 0
        end
        object leCppLibPath: TLabeledEdit
          Left = 5
          Top = 130
          Width = 346
          Height = 21
          EditLabel.Width = 58
          EditLabel.Height = 13
          EditLabel.Caption = 'Library path'
          TabOrder = 1
        end
        object leCppOutPath: TLabeledEdit
          Left = 5
          Top = 180
          Width = 346
          Height = 21
          EditLabel.Width = 175
          EditLabel.Height = 13
          EditLabel.Caption = 'Output path for IHX, OMF, ASM etc.'
          TabOrder = 2
        end
        object leCppModPath: TLabeledEdit
          Left = 5
          Top = 30
          Width = 346
          Height = 21
          EditLabel.Width = 64
          EditLabel.Height = 13
          EditLabel.Caption = 'Modules path'
          TabOrder = 3
        end
      end
    end
  end
  object pnBottom: TPanel
    Left = 0
    Top = 249
    Width = 411
    Height = 41
    Align = alBottom
    BevelOuter = bvNone
    ParentColor = True
    TabOrder = 1
    DesignSize = (
      411
      41)
    object Label1: TLabel
      Left = 10
      Top = 5
      Width = 181
      Height = 31
      AutoSize = False
      Caption = 'Empty paths will be replaced by path of source file'
      WordWrap = True
    end
    object CancelBtn: TBitBtn
      Left = 315
      Top = 5
      Width = 86
      Height = 31
      Anchors = [akRight, akBottom]
      Cancel = True
      Caption = 'Cancel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0C0DCC0}
      ModalResult = 2
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
    end
    object OKBtn: TBitBtn
      Left = 225
      Top = 5
      Width = 86
      Height = 31
      Anchors = [akRight, akBottom]
      Caption = 'OK'
      Default = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        DCC0C0DCC0C0DCC0ADC4AD696B69777E77AFC6AFC0DCC0C0DCC0}
      ModalResult = 1
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
    end
  end
end
