...
type
  ...
  private
    ...
    procedure LoadHistory (AIniFile : TMemIniFile; ASection : string; ACombo   : TComboBox);
    procedure SaveHistory (AIniFile : TMemIniFile; ASection : string; ACombo   : TComboBox);
...
begin
  ...
  IniFile:=TMemIniFile.Create(AIniName);
...
{ ---------------------------------------------------------------- }
procedure TFindReplDialog.LoadHistory (AIniFile : TMemIniFile;
                                       ASection : string;
                                       ACombo   : TComboBox);
var
  i : integer;
...
procedure TFindReplDialog.SaveHistory (AIniFile : TMemIniFile;
                                       ASection : string;
                                       ACombo   : TComboBox);
  ...
  begin
    n:=s.IndexOf(#13);
    if n<0 then Result:=s else Result:=Trim(copy(s,1,n));
    end;
...
begin
  with AIniFile do begin
    ...
    with ACombo.Items do for i:=0 to Count-1 do
      WriteString(ASection,iniHist+IntToStr(i),GetFirstLine(Strings[i]));
    end;
  end;
